...
def _atr(prices: pd.DataFrame, n: int = 14) -> float:
    ...
    # fallback: proxie met std van dag-returns
    c = pd.to_numeric(prices["Close"], errors="coerce").dropna()
    rets = c.pct_change().dropna()
    return float(c.iloc[-1] * rets.std() * np.sqrt(1)) if len(rets) else float("nan")
...
def system_actions(prices: Dict[str, pd.DataFrame], params: Dict[str, Any], system_cfg: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Bouw regel-gebaseerde orders:
    - signaal via bestaande signalen + (optioneel) regime/MACD/hysterese
    - sizing via ATR of simpele vol
    - output: lijst met dicts {ticker, action, size, entry, stop, take_profit, notes}
    """
    ...
    # bouw indicatoren per ticker
    ind_map: Dict[str, pd.DataFrame] = {}
    for t, df in prices.items():
        ind_map[t] = indicators(df, params)

    # rangschik tickers op momentum (bijv. SMA_S premium)
    scored = []
    ...
    for t in tickers:
        ...
        # sizing
        atr_val = _atr(prices.get(t), atr_n) if use_atr else float("nan")
        risk_unit = atr_val if math.isfinite(atr_val) else (close * 0.03)  # ~3% band als fallback
        risk_per_share = max(1e-6, risk_unit)
        euro_risk = max(1.0, capital * risk_pct)  # risicobudget per trade
        ...
        actions.append({
            "ticker": t,
            "action": "LONG" if base == "BUY" else "FLAT",  # systeem is long/flat
            "entry": round(entry, 4),
            "size": int(size),
            "stop": round(stop, 4),
