...
# ---- LLM backends (choose via env: LLM_PROVIDER) ----
def _chat_openai(system: str, user: str) -> str:
    try:
        from openai import OpenAI
    except Exception:
        return "AI is niet geactiveerd (openai package ontbreekt)."
...
def _chat_groq(system: str, user: str) -> str:
    try:
        ...
        return "Groq client niet geïnstalleerd. Verwijder LLM_PROVIDER=groq of installeer groq."
...
def _chat_gemini(system: str, user: str) -> str:
    # Gebruik officiële lib als aanwezig; anders meld duidelijk
    try:
        import google.generativeai as genai  # optional
    except Exception:
        return "Gemini client niet geïnstalleerd. Verwijder LLM_PROVIDER=gemini of installeer google-generativeai."
...
def chat_answer(user_msg: str, provider: str = "auto") -> Dict[str, Any]:
    ...
    for it in news_items[:10]:
        title = it.get("title",""); pub = it.get("publisher",""); link = it.get("l
