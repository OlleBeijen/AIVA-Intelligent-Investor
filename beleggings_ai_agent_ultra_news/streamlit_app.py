...
# ---- Make sure 'src' is importable (works from repo root or subdir) ----
...
# NEW: Value + ML
from src.value_screen import compute_scores
from src.ml_forecast import forecast_ml
...
with st.sidebar:
    ...
    st.divider()
    st.write("### DCF-parameters (Value Radar)")
    dcf_discount = st.slider("Discount rate", 0.06, 0.15, 0.10, 0.005)
    dcf_growth   = st.slider("Startgroei FCF", 0.00, 0.08, 0.04, 0.005)
    st.caption("Conservatief houden = beter signaal.")

# ---- Tabs (single definition) ----
tab_names = [
    "📊 Overview", "📈 Forecast + bronnen", "💬 AI Chat", "🗞️ News",
    "🔎 Screener", "🧭 Value Radar", "🤖 ML Forecast", "🧪 Strategy Lab",
    "📦 Portfolio tools", "🎯 Doelen & Risico", "📥 Portefeuille import",
    "🔒 Compliance", "⚙️ Config Wizard", "📝 Journal", "❓ Help"
]
...
# ------- Value Radar -------
with tabs[5]:
    st.subheader("Value Radar (Buffett-stijl)")
    universe = sorted(set(cfg.get("watchlist", []) + cfg["portfolio"]["tickers"]))
    st.caption(f"Universum: {len(universe)} tickers")
    if st.button("Run Value-scan"):
        prices = fetch_prices(universe, lookback_days=300)
        df = compute_scores(universe, dcf_growth=dcf_growth, dcf_discount=dcf_discount)
        if not df.empty:
            # Uptrend check (200d)
            up = {}
            for t in df["ticker"]:
                d = prices.get(t)
                if d is not None and "Close" in d and len(d) > 200:
                    px = d["Close"]
                    ma200 = px.rolling(200).mean().iloc[-1]
                    up[t] = float(px.iloc[-1] > ma200)
                else:
                    up[t] = 0.0
            df["Uptrend200"] = df["ticker"].map(up)

            mos_min = st.slider("Min. Margin of Safety", -0.5, 1.0, 0.2, 0.05)
            want_up = st.checkbox("Alleen in uptrend (200d)", value=True)
            q = df.copy()
            q = q[q["MarginOfSafety"].fillna(-9) >= mos_min]
            if want_up:
                q = q[q["Uptrend200"] >= 1.0]

            nice = q[["ticker","name","price","PE","PB","EV/EBITDA","IntrinsicValue","MarginOfSafety","Quality","Uptrend200","currency","source"]]
            st.dataframe(nice.sort_values(["MarginOfSafety","ValueScore","Quality"], ascending=[False, False, False]), use_container_width=True)

            # Scatter plot
            try:
                import plotly.express as px
                sc = q.dropna(subset=["ValueScore","Quality"])
                if not sc.empty:
                    fig = px.scatter(sc, x="ValueScore", y="Quality", color="MarginOfSafety", hover_name="ticker",
                                     title="Value vs Quality (kleur = Margin of Safety)")
                    st.plotly_chart(fig, use_container_width=True)
            except Exception:
                pass
            st.caption("Klik op de bron-link in de tabel voor de bedrijfswebsite/factsheet.")
        else:
            st.info("Geen fundamentals gevonden voor deze tickers.")

# ------- ML Forecast -------
with tabs[6]:
    st.subheader("ML Forecast (5 dagen)")
    tickers = cfg["portfolio"]["tickers"]
    prices = fetch_prices(tickers, lookback_days=400)
    if prices:
        res = forecast_ml(prices, horizon=5)
        if res:
            df = pd.DataFrame.from_dict(res, orient="index")
            df.index.name = "ticker"
            df = df.reset_index().sort_values("exp_return_5d", ascending=False)
            st.dataframe(df, use_container_width=True)
            st.caption("Model: Gradient Boosting op prijsfeatures (lags, MA-gaps, vol, RSI). Cross-val per ticker.")
        else:
            st.info("Niet genoeg data om te trainen.")
    else:
        st.info("Geen prijzendata.")
